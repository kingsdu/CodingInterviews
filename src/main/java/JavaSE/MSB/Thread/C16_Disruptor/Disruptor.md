...
链表遍历肯定比数组慢，但是JDK中没有ConcurrentArrayQueue？因为数组的大小的固定的，每次操作都要copy数组，这样性能很低。

Disruptor是数组实现的，首位相连，所以牛。

用Sequence作为环形数组中的环形指针。
...
生产者满了，sequence中有指针中有标注某个数据是否能够被覆盖，不能则执行对于的等待策略。
